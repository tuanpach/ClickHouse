...
SELECT countIf(ProfileEvents['ParallelReplicasQueryCount']>0) >= 1 FROM system.query_log
...
SELECT countIf(ProfileEvents['ParallelReplicasQueryCount']>0) >= 2 FROM system.query_log
WHERE type = 'QueryFinish' AND event_date >= yesterday()
